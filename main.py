...
if len(sys.argv) <= 1:
    print(
    "{0}: needs a filepath to a PCAP file".format(sys.argv[0]))
    sys.exit(-1)
...
if len(sys.argv) == 3:
    try:
        ...
        print(
        "invalid ratio value:", sys.argv[2])
        sys.exit(-1)
...
for s in sorted(suspects.keys(), cmp=compare_IPs):
    ...
    print(
    "{0:15} had {1} SYNs and {2} SYN-ACKs".format(s, syns, synacks))
